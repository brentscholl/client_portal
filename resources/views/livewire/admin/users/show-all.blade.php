<div>
    <div class="grid grid-cols-12 mb-4">
        ...
        <div class="col-span-2 sm:col-span-2 pr-2">
            <x-input.select
        ...
        <div class="col-span-2 pr-2">
            <x-input.select
                ...
                label="Type"
            >
                <x-input.option>Choose a Type</x-input.option>
                <x-input.option value="admin">Admin</x-input.option>
                <x-input.option value="client">Client</x-input.option>
            </x-input.select>
        </div>
        <div class="col-span-2 pr-2">
            <x-input.select
        ...
        <div class="col-span-5">
            <x-input.text wire:model="search" label="Search" placeholder="Search Users..."/>
        </div>
    </div>
            ...
            <div class="align-middle inline-block min-w-full shadow sm:rounded-lg border-b border-gray-200">
                <table class="s-table">
                    ...
                    <tr>
                        <th scope="col">
                            <button wire:click="sortBy('first_name')"
                                class="group flex items-center space-x-2 text-xs font-medium uppercase tracking-wider {{ $sortField == 'first_name' ? 'text-gray-900 underline' : 'text-gray-500' }} hover:text-gray-900 ">
                                <span>Name</span>
                    ...
                    @foreach($users as $user)
                        @livewire('admin.users.user-row', [$user], key('user-row-'.$user->id))
                    @endforeach
                    </tbody>
                </table>
            </div>
