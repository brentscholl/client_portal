<section class="mt-8 xl:mt-10">
    <x-card :opened="$cardOpened" :cardOpened="$cardOpened">
        <x-card.header title="Packages" :count="$packages->total()">
            @livewire('admin.packages.create', ['service' => $service,], key('create-package'))
        </x-card.header>
                ...
                @if($cardOpened)
                    @if($packages->count() > 0)
                        <div class="grid grid-cols-1 gap-4">
                            @foreach($packages as $package)
                                @livewire('admin.packages.package-card', ['package' => $package, 'showService' => $showService], key($package->id))
                            @endforeach
                        </div>
                    ...
                    @else
                        <span class="relative block w-full border-2 border-gray-200 border-dashed rounded-lg p-12 text-center">
                            <x-svg.package class="mx-auto h-12 w-12 text-gray-200"/>
