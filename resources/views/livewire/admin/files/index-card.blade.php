<section class="mt-8 xl:mt-10">
    <x-card :opened="$cardOpened">
        <x-card.header title="Files" count="{{ optional($files)->total() }}">
            @if($allowUpload)
                @livewire('admin.files.upload', ['model' => $model, 'setClient' => $setClient, 'client_id' => $client_id, 'is_resource' => false], key('upload-file'))
            @endif
        </x-card.header>
                    ...
                    @if($files->count() > 0)
                        <div class="grid grid-cols-1 gap-4">
                            @foreach($files as $file)
                                @livewire('admin.files.file-card', ['file' => $file], key($file->id))
                            @endforeach
                        </div>
                    ...
                    @else
                        <span class="relative block w-full border-2 border-gray-200 border-dashed rounded-lg p-12 text-center">
                            ...
                            <span class="mt-2 block text-sm font-medium text-gray-300">
                              No files uploaded to {{ str_replace('App\Models\\', '', get_class($model)) }}
                            </span>
                        </span>
                    @endif
                @else
