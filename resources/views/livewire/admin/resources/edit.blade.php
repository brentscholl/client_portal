<x-slideout.wrapper>
    ...
    <form wire:submit.prevent="saveResource" autocomplete="off">
        <x-slideout
            title="Edit Resource"
            subtitle="{{ $resource->label }}"
            saveBtn="Update Resource"
        >
            @include('errors.list')
            <div x-data="{ 'type': @entangle('type') }" class="space-y-6 pt-6 pb-5 text-left">
                @if($slideout_open)
                    <div>
                        @switch($type)
                            @case('text')
                            ...
                            @break
                        @endswitch
                        <x-input.radio-set index="{{ $type_index }}" label="Resource Type">
                            <x-input.radio
                                wire:model="type"
                                value="text"
                                label="Text Input"
