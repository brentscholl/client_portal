    ...
    <div class="grid grid-cols-12 mb-4">
        ...
        <div class="col-span-2 pr-2">
            <x-input.select
                ...
                label="Package"
            >
                <x-input.option>Choose a Package</x-input.option>
                @foreach($packages as $key => $package)
                    <x-input.option value="{{$package->id}}">{{$package->title}}</x-input.option>
                @endforeach
            </x-input.select>
        </div>

        <div class="col-span-5">
            <x-input.text wire:model="search" label="Search" placeholder="Search Tutorials..."/>
        </div>
    </div>
            ...
            <div class="align-middle inline-block min-w-full shadow sm:rounded-lg border-b border-gray-200">
                <table class="s-table">
                    ...
                    @foreach($tutorials as $tutorial)
                        @livewire('admin.tutorials.tutorial-row', [$tutorial], key('tutorial-row-'. $tutorial->id))
                    @endforeach
                    </tbody>
                </table>
            </div>
