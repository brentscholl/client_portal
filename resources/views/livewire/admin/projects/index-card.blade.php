<section class="mt-8 xl:mt-10">
    <x-card :opened="$cardOpened" :cardOpened="$cardOpened">
        <x-card.header title="Projects" :count="$projects->total()">
            @if(!$user)
                ...
                @if($cardOpened)
                            ...
                            <div class="flex space-between space-x-2 items-center">
                                ...
                                <div>
                                    <div class="hidden ml-6 bg-gray-100 p-0.5 rounded-lg items-center sm:flex">
                                        <button tooltip="Show visible Projects" wire:click="setVisibleFilter('visible')" type="button" class="@if($filter_visible == 'visible') bg-white shadow-sm text-green-500 @else text-gray-400 hover:bg-white hover:shadow-sm @endif p-1.5 rounded-md focus:outline-none focus:ring-2 focus:ring-inset focus:ring-secondary-500">
                                            <x-svg.eye class="h-4 w-4"/>
                                        </button>
                                        <button tooltip="Show hidden Projects" wire:click="setVisibleFilter('hidden')" type="button" class="@if($filter_visible == 'hidden') bg-white shadow-sm text-red-500 @else text-gray-400 hover:bg-white hover:shadow-sm @endif ml-0.5 p-1.5 rounded-md text-gray-400 focus:outline-none focus:ring-2 focus:ring-inset focus:ring-secondary-500">
                    ...
                    <div class="grid grid-cols-1 gap-4">
                        @foreach($projects as $project)
                            @livewire('admin.projects.project-card', ['project' => $project, 'showService' => $showService], key($project->id))
                        @endforeach
                    </div>
