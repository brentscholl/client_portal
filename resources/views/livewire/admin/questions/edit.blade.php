<x-slideout.wrapper>
    ...
    <form wire:submit.prevent="saveQuestion" autocomplete="off">
        <x-slideout
            title="Edit Question"
            subtitle="{{ optional($service)->title ?  optional($service)->title : (optional($client)->title ? optional($client)->title : '') }} {{ optional($package)->title ? ' - ' . optional($package)->title : '' }}"
            saveBtn="Update Question"
        >
            @include('errors.list')
            <div class="space-y-6 pt-6 pb-5">
                @if($slideout_open)
                    ...
                    <div x-data="{ showInput: @entangle('question_type') }">
                        <x-input.radio-set index="{{ $type_index }}" label="Question Type">
                            <x-input.radio
