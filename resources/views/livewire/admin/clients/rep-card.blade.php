<div class="col-span-1 bg-white {{ $user->id == $client->primary_contact ? 'border border-green-100' : '' }} rounded-lg border relative">
    @if($user->id == $client->primary_contact)
        <span
    ...
    <div class="divide-y divide-gray-200">
        <div class="flex-1 flex items-center justify-between relative">
            <a tooltip="View User" href="{{ route('admin.users.show', $user->id) }}"
                class="group w-full flex items-center justify-between p-6 space-x-6">
    ...
    <x-menu.three-dot :showMenuContent="$showMenuContent">
        @if($showMenuContent)
            <a tooltip="Sign in as this User" tooltip-p="left" wire:click="impersonate({{$user->id}})" href="#"
                class="group block w-full text-left px-4 py-2 text-sm text-secondary-600 hover:text-secondary-900">Impersonate</a>
            @livewire('admin.users.edit', ['user' => $user, 'button_type' => 'inline-menu'], key('user-edit-' . $user->id))
            @if($client->primary_contact != $user->id)
                <x-button
                    btn="inline-menu"
                    wire:click="makePrimaryContact"
                >
                    Make primary contact for client
                </x-button>
            @endif
            <x-modal
                wire:click="destroy"
