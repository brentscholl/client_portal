...
<div x-data="{ open: false, select: null}" x-init="">
    <label id="service-list-label-{{ $uid }}" class="{{ $required ? 'required' : '' }}">
        Assigned to Service
    </label>
    <div class="mt-1 relative">
        ...
        >
            @if($service)
                <span class="block truncate">{{ $service->title }}</span>
            @else
                <span class="block truncate">Select a Service</span>
            @endif
...
{{--                        id="listbox-option-{{ $loop->index }}"--}}
                        wire:click='setService("{{ $s->id }}")'
                        @click="open = false"
                        @if(optional($service)->id == $s->id)
                            class="w-full relative inline-flex space-x-2 items-center justify-start py-2 px-4 border border-transparent text-primary-500 bg-white shadow-sm text-sm font-medium rounded-md hover:bg-secondary-500 hover:text-white focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-secondary-500"
                        @else
                            class="w-full relative inline-flex space-x-2 items-center justify-start py-2 px-4 border border-transparent bg-white shadow-sm text-sm font-medium rounded-md hover:bg-secondary-500 hover:text-white focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-secondary-500"
                        @endif
                    >
                        <x-dynamic-component :component="'svg.service.'.$s->slug" class="h-6 w-6 text-gray-300" />
                        <span
                            @if(optional($service)->id == $s->id)
