...
<div x-data="{ open: false, select: null}" x-init="">
    <label id="phase-select-{{ $uid }}" class="{{ $required ? 'required' : '' }}">
        Assign to Phase
    </label>
    <div class="mt-1 relative">
        <button
            ...
            @if(!$project) disabled="disabled" @endif
        >
            @if($phase)
                <span class="block truncate cursor-pointer">{{ $phase->title }}</span>
            @else
                <span class="block truncate cursor-pointer">Select a Phase</span>
            @endif
        ...
        >
            ...
            >
                ...
                @if($project)
                    @foreach($project->phases as $s)
                        <button
                            wire:key="{{ $loop->index }}-{{ $uid }}"
                            type="button"
                            wire:click='setPhase("{{ $s->id }}"{{ $i ? ', ' . $i : '' }})'
                            @click="open = false"
                            @if(optional($phase)->id == $s->id)
