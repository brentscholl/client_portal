...
<div x-data="{ open: false, select: null}" x-init="">
    ...
    <div class="mt-1 relative">
        ...
        >
            @if($client)
                <span class="block truncate cursor-pointer">{{ $client->title }}</span>
            @else
                <span class="block truncate cursor-pointer">Select a Client</span>
            @endif
...
{{--                        id="listbox-option-{{ $loop->index }}"--}}
                        wire:click='setClient("{{ $c->id }}")'
                        @click="open = false"
                        @if(optional($client)->id == $c->id)
