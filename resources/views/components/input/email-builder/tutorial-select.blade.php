...
<div x-data="{ open: false, select: null}" x-init="">
    ...
    <div class="mt-1 relative">
        <button
            ...
            @if(!$tutorials) disabled="disabled" @endif
        >
            @if($tutorialId)
                <span class="block truncate cursor-pointer">{{ $tutorialTitle }}</span>
            @else
                <span class="block truncate cursor-pointer">Select Tutorial</span>
            @endif
                ...
                @if($tutorials)
                    @foreach($tutorials as $t)
                        <button
                            ...
                            type="button"
                            wire:click="setTutorial({{ $t['id'] }}, '{{ $t['title'] }}', {{ $i }})"
                            @click="open = false"
                            @if($tutorialId== $t['id'])
