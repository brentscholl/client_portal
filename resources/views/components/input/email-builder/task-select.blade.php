...
<div x-data="{ open: false, select: null}" x-init="">
    ...
    <div class="mt-1 relative">
        <button
            ...
            @if(!$tasks) disabled="disabled" @endif
        >
            @if($taskId)
                <span class="block truncate cursor-pointer">{{ $taskTitle }}</span>
            @else
                <span class="block truncate cursor-pointer">Select Task</span>
            @endif
                ...
                @if($tasks)
                    @foreach($tasks as $t)
                        <button
                            ...
                            type="button"
                            wire:click="setTask({{ $t['id'] }}, '{{ $t['title'] }}', {{ $i }})"
                            @click="open = false"
                            @if($taskId== $t['id'])
