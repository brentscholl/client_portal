...
<div x-data="{ open: false, select: null}" x-init="">
    ...
    <div class="mt-1 relative">
        <button
            ...
            @if(!$questions) disabled="disabled" @endif
        >
            @if($questionId)
                <span class="block truncate cursor-pointer">{{ $questionBody }}</span>
            @else
                <span class="block truncate cursor-pointer">Select Question</span>
            @endif
                ...
                @if($questions)
                    @foreach($questions as $t)
                        <button
                            ...
                            type="button"
                            wire:click="setQuestion({{ $t['id'] }}, '{{ $t['body'] }}', {{ $i }})"
                            @click="open = false"
                            @if($questionId== $t['id'])
