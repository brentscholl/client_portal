...
<div x-data="{ open: false, select: null}" x-init="" class="{{ $attributes->get('class') }}">
    <label for="answer-select-{{ $uid }}">
        {{ $label }}
    </label>
    <div class="mt-1 relative">
        <button
            ...
            @if(!$answers) disabled="disabled" @endif
        >
            @if($answer)
                <span class="block truncate">{{ Str::limit($answer->question->body, 30) }}: {{ Str::limit($answer->answer, 30) }}</span>
            @else
                <span class="block truncate">Select an Answer</span>
            @endif
            ...
            >
                ...
                @foreach($answers as $t)
                    <button
                        ...
                        type="button"
                        wire:click='setAnswer("{{ $t->id }}")'
                        @click="open = false"
                        @if(optional($answer)->id == $t->id)
                    ...
                    >
                        <span
                            ...
                            @endif
                        >
                            {{ Str::limit($t->question->body, 30) }}: {{ Str::limit($t->answer, 30) }}
                        </span>
